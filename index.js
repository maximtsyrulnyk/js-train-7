...
function createSymbolWithDescription(property) {
  let symbol = Symbol(property);// Створюємо символ symbol з описом property
...
function createSymbolForKey(key) {
  let symbol = Symbol.for(key);// Використовуємо Symbol.for() для створення нового символу symbol зі значенням key
...
let rangeObject = {
  ...
  next() {
    ...
    // та не забуваємо збільшити індекс за допомогою інкременту, і "done" - false, означаючи, що ітерація ще не закінчена
...
let myObject = {
  ...
  next() {
    ...
    if (this.currentIndex < this.category.length) {
      // Створюємо змінну value, якій присвоюємо властивість name елемента масиву category з індексом currentIndex
      let value = this.category[this.currentIndex].name;

      // Збільшуємо currentIndex на одиницю
      this.currentIndex++;
